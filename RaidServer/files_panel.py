...
class FilesPanel(wx.lib.scrolledpanel.ScrolledPanel):
    ...
    def __init__(self, parent, set_ending_bmp, get_icon, get_objects, screenWidth, screenHeight, is_data_server, OnButton_Press):
        self.screenWidth = screenWidth
        self.screenHeight = screenHeight
        self.is_data_server = is_data_server
        wx.lib.scrolledpanel.ScrolledPanel.__init__(self, parent, -1,
                                                    size=(self.screenWidth, self.screenHeight * 0.9),
                                                    pos=(0, self.screenHeight * 0.11),
                                                    style=wx.SIMPLE_BORDER)
        self.PIC_SIZE = self.screenWidth / 11
        self.SetupScrolling()
    ...
    def show_files(self, search=None):
        ...
        if search:
            file_show = [x for x in self.get_objects() if search in x]
        else:
            file_show = self.get_objects()
