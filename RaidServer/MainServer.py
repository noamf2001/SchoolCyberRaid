...
class MainServer():
    def __init__(self, saving_path=""):
        ...
        self.data_server_command_def = {
            -1: self.disconnect_data_server}  # msg_type : method that take care of it, take as parameter: current
    ...
    def sign_up_data_server(self, current_socket, msg_parameter):
        print "main server: sign up data server:  "
    ...
    def upload_file(self, current_socket, msg_parameters):
        ...
        print "division of the file: " + str(division_part_data_server)
        for i in range(len(division_part_data_server)):
            for j in range(division_part_data_server[i][1]):
    ...
    def main(self):
        while True:
            ...
            if not self.data_server_command.empty():
                ...
                print "result: " + str(result)
                print "\n"
                if command_data_server[1][0] != -1 and command_data_server[1][0] != 1:
                    self.command_result_data_server.put([command_data_server[0], [command_data_server[1][0], result]])
